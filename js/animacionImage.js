...
function handleScroll() {
  const images = document.querySelectorAll(".about__img"); // Seleccionamos todas las imágenes con la clase .about__img

  images.forEach((image) => {
    if (isInViewport(image)) {
      // Si la imagen entra en el viewport, agregamos la clase "visible"
      image.classList.add("about__img--visible");
    } else {
      // Si la imagen ya no está en el viewport, eliminamos la clase "visible"
      image.classList.remove("about__img--visible");
    }
  });
}
